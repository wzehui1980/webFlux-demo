...
技术栈说明：Spring Boot 3.2.x（WebFlux 依赖）、Java 17+、Lombok（简化代码）、Postman（接口测试）、WebClient（客户端调用）、H2 内存数据库、R2DBC（响应式数据库访问）、Spring Security（安全认证）、Thymeleaf（模板引擎）、Netty（响应式容器）。
...
**R2DBC（Reactive Relational Database Connectivity）** 是响应式关系型数据库连接规范，专为 Spring WebFlux 等响应式应用设计。与传统的 JDBC 不同，R2DBC 提供完全非阻塞的数据库访问能力，能够充分发挥 WebFlux 的性能优势。

**为什么选择 R2DBC 而不是 JPA？**
- ✅ **真正的响应式**：JDBC 是阻塞的，即使包装成 Mono/Flux，底层仍然是阻塞调用
- ✅ **高性能**：非阻塞 I/O，适合高并发场景
- ✅ **背压支持**：自动处理数据流速率不匹配问题
- ✅ **资源利用率高**：少量线程即可处理大量并发请求

**注意事项：**
- ⚠️ R2DBC 不支持懒加载、级联操作等 JPA 特性
- ⚠️ 需要手动管理实体关系（如多对多）
- ⚠️ 部分高级查询功能需要通过自定义 SQL 实现

## 3.2 Schema 初始化（schema.sql）

在 `src/main/resources/schema.sql` 中定义数据库表结构：
...
public class UserEntity {
    ...
    // 注意：R2DBC 不支持 @ManyToMany，需要手动管理关联关系
    @Transient
    private Set<RoleEntity> roles = new HashSet<>();
}
```

### 3.3.2 角色实体（RoleEntity）
...
- `@Table`：指定数据库表名
- `@Id`：标记主键字段
- `@Column`：指定列名（可选，默认使用字段名）
- `@Transient`：标记非持久化字段（不会映射到数据库）

## 3.4 响应式 Repository 接口

### 3.4.1 用户仓库（UserRepository）
...
public interface UserRepository extends ReactiveCrudRepository<UserEntity, Long> {
    ...
    Flux<UserEntity> findByActive(Boolean active);
    
    // 自定义查询：根据年龄范围查询
    @Query("SELECT * FROM users WHERE age BETWEEN :minAge AND :maxAge")
    Flux<UserEntity> findByAgeRange(Integer minAge, Integer maxAge);
}
```

### 3.4.2 角色仓库（RoleRepository）
...
**ReactiveCrudRepository 提供的默认方法：**
- `Mono<T> findById(ID id)`：根据 ID 查询
- `Flux<T> findAll()`：查询所有
- `Mono<T> save(T entity)`：保存或更新
- `Mono<Void> deleteById(ID id)`：根据 ID 删除
- `Mono<Void> delete(T entity)`：删除实体
- `Mono<Long> count()`：统计数量
- `Mono<Boolean> existsById(ID id)`：判断是否存在

## 3.5 Service 层实现

### 3.5.1 用户服务（UserService）
    ...
    public Mono<UserEntity> save(UserEntity user) {
        return userRepository.findByName(user.getName())
                .flatMap(existingUser -> {
                    ...
                    return Mono.error(new RuntimeException("用户名 '" + user.getName() + "' 已存在"));
                })
...
**关键知识点：**
- `.flatMap()`：异步转换，用于链式调用
- `.switchIfEmpty()`：处理空值情况
- `.defer()`：延迟执行，避免提前订阅
- `LocalDateTime.now()`：手动管理时间字段（R2DBC 不支持 JPA 的自动审计）

## 3.6 数据初始化（DataInitializer）

使用 Java 代码而非 SQL 文件进行数据初始化，避免编码问题：
    ...
    public void run(String... args) throws Exception {
        ...
        roleRepository.count()
                ...
                .flatMapMany(count -> {
                    ...
                    RoleEntity adminRole = new RoleEntity(null, "ADMIN", "系统管理员，拥有所有权限", now, now);
                    RoleEntity userRole = new RoleEntity(null, "USER", "普通用户，拥有基本权限", now, now);
                    RoleEntity managerRole = new RoleEntity(null, "MANAGER", "经理，拥有管理权限", now, now);
                    
                    return roleRepository.saveAll(Flux.just(adminRole, userRole, managerRole));
                })
                .thenMany(userRepository.count())
                .filter(count -> count == 0)
                .flatMapMany(count -> {
                    log.info("创建初始用户...");
                    LocalDateTime now = LocalDateTime.now();
                    ...
                    adminRoles.add(new RoleEntity(1L, "ADMIN", "系统管理员，拥有所有权限", now, now));
                    UserEntity admin = new UserEntity(null, "admin", 30, "13800000000", 
                                                    "admin@example.com", true, now, now, adminRoles);
                    
                    // 创建普通用户
                    Set<RoleEntity> userRoles = new HashSet<>();
                    userRoles.add(new RoleEntity(2L, "USER", "普通用户，拥有基本权限", now, now));
                    UserEntity user1 = new UserEntity(null, "张三", 20, "13800138000", 
                                                     "zhangsan@example.com", true, now, now, userRoles);
                    
                    return userRepository.saveAll(Flux.just(admin, user1));
                })
                .doOnComplete(() -> log.info("数据初始化完成！"))
                .subscribe();
    }
}
```
...
| 特性 | R2DBC | JPA/Hibernate |
|------|-------|---------------|
| **编程模型** | 响应式（Mono/Flux） | 阻塞式 |
| **性能** | 高（非阻塞 I/O） | 一般（阻塞 I/O） |
| **并发能力** | 强（少量线程处理大量请求） | 弱（需要线程池） |
| **学习曲线** | 较陡（需理解响应式编程） | 平缓 |
| **懒加载** | ❌ 不支持 | ✅ 支持 |
| **级联操作** | ❌ 需手动管理 | ✅ 自动处理 |
| **多对多关系** | ❌ 需手动管理中间表 | ✅ 自动处理 |
| **自动审计** | ❌ 需手动设置时间字段 | ✅ @CreatedDate/@LastModifiedDate |
| **生态系统** | 发展中 | 成熟 |
| **适用场景** | 高并发 IO 密集型应用 | 传统 CRUD 应用 |

**选择建议：**
- ✅ **高并发场景**：选择 R2DBC（如即时通讯、实时推送）
- ✅ **微服务架构**：选择 R2DBC（更好的资源利用率）
- ✅ **传统企业应用**：选择 JPA（开发效率高，生态成熟）
- ✅ **混合使用**：可以在 WebFlux 项目中使用 JPA（通过 `Mono.fromCallable()` 包装）
...
Flux<UserEntity> findByPage(@Param("limit") int limit, @Param("offset") int offset);
```

### Q2: 如何处理多对多关系？

A: R2DBC 不支持自动管理多对多关系，需要手动操作中间表：
```java
// 保存用户-角色关联
@Query("INSERT INTO user_roles (user_id, role_id) VALUES (:userId, :roleId)")
Mono<Void> assignRoleToUser(@Param("userId") Long userId, @Param("roleId") Long roleId);
...
A: 使用 `saveAll()` 方法：
```java
userRepository.saveAll(Flux.just(user1, user2, user3))
    .doOnComplete(() -> log.info("批量插入完成"))
    .subscribe();
```

### Q4: R2DBC 支持事务吗？

A: 支持，使用 `@Transactional` 注解：
```java
@Transactional
public Mono<UserEntity> createUserWithRole(UserEntity user, Long roleId) {
    return userRepository.save(user)
            .flatMap(savedUser -> assignRoleToUser(savedUser.getId(), roleId))
...
> **注意**：本章展示的是传统 JPA 方式的实体类设计，如果您使用 R2DBC（推荐），请参考第三章中的 R2DBC 实体类设计。
...
> **注意**：本章展示的是传统 JPA 方式的 Repository 设计，如果您使用 R2DBC（推荐），请参考第三章中的 R2DBC Repository 设计。

## 5.1 用户仓库（UserRepository）
...
## 6.1 HelloController - Mono/Flux 基础示例
...
## 6.2 UserController - 用户管理 REST API
...
> **注意**：以下展示的是传统 JPA 方式的 Service 实现，如果您使用 R2DBC（推荐），请参考第三章中的 R2DBC Service 实现。

## 8.1 用户服务（UserService）
...
# 九、安全配置（Spring WebFlux Security）
...
- **R2DBC 响应式数据库访问**：真正的非阻塞 I/O，高性能数据库操作
- **手动时间字段管理**：在 Service 层自动维护 createTime 和 updateTime
- **Java 代码数据初始化**：避免 SQL 文件编码问题，支持日志输出
- **用户-角色多对多关系**：通过中间表实现灵活的权限控制
- **分页查询**：Spring Data R2DBC 支持自定义分页查询
...
## Q1: R2DBC 和 JPA 有什么区别？应该选择哪个？

A: 
- **R2DBC**：真正的响应式数据库访问，非阻塞 I/O，适合高并发场景，但需要手动管理实体关系和时间字段。
- **JPA**：成熟的 ORM 框架，自动管理实体关系和懒加载，但底层是阻塞的 JDBC。
- **选择建议**：高并发 IO 密集型应用选择 R2DBC；传统 CRUD 应用选择 JPA；也可以在 WebFlux 项目中通过 `Mono.fromCallable()` 包装 JPA 调用。
...
## Q7: 为什么使用 @EnableWebFluxSecurity 而不是 @EnableWebSecurity？
...
## Q11: R2DBC 中如何处理中文乱码问题？

A: 
1. **确保全链路 UTF-8 编码**：
   - Maven 配置：在 `pom.xml` 中设置 `<project.build.sourceEncoding>UTF-8</project.build.sourceEncoding>`
   - JVM 参数：创建 `.mvn/jvm.config` 文件，添加 `-Dfile.encoding=UTF-8`
   - 编辑器配置：创建 `.editorconfig` 文件，指定 `charset = utf-8`
2. **使用 Java 代码初始化数据**：避免 SQL 文件的编码问题，Java 源文件天然 UTF-8 编码
3. **数据库连接 URL**：确保 R2DBC URL 正确配置（H2 默认支持 UTF-8）

## Q12: 为什么编辑保存后更新时间显示为空？

A: R2DBC 不支持 JPA 的 `@LastModifiedDate` 自动审计功能，需要在 Service 层手动设置：
```java
public Mono<RoleEntity> save(RoleEntity role) {
    return roleRepository.findByName(role.getName())
        .flatMap(existingRole -> {
            if (role.getId() != null && existingRole.getId().equals(role.getId())) {
                // 更新操作：保留创建时间，设置更新时间
                role.setCreateTime(existingRole.getCreateTime());
                role.setUpdateTime(LocalDateTime.now()); // 手动设置
                return roleRepository.save(role);
            }
            // ...
        })
